import { LayoutDashboard, FlaskConical, FolderOpen, Play, Users, ChevronRight, LogOut } from 'lucide-react'
...
export default function Sidebar({ currentView, onNavigate, testCaseCount, testSuiteCount, onSignOut }: SidebarProps) {
  ...
  return (
    <aside className="fixed inset-y-0 left-0 w-64 bg-zinc-900 border-r border-zinc-800 flex flex-col z-20">
      {/* Logo */}
      <div className="h-16 flex items-center gap-3 px-5 border-b border-zinc-800">
        ...
        <div className="min-w-0">
          <p className="text-sm font-semibold text-white truncate">QA Manager</p>
      ...
      <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
        ...
        {NAV_ITEMS.map((item) => {
          ...
          return (
            ...
            >
              <Icon className={cn('w-4 h-4 flex-shrink-0', isActive ? 'text-white' : 'text-zinc-500 group-hover:text-zinc-300')} />
      ...
      <div className="px-3 py-4 border-t border-zinc-800 space-y-2">
        {onSignOut && (
          <button
            onClick={onSignOut}
            className="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium text-zinc-500 hover:text-zinc-200 hover:bg-zinc-800 transition-colors"
          >
            <LogOut className="w-4 h-4 flex-shrink-0" />
            <span>Sign Out</span>
          </button>
        )}
        <p className="px-3 text-xs text-zinc-600">v0.1.0 &mdash; Supabase</p>
      </div>
    </aside>
  )
}
