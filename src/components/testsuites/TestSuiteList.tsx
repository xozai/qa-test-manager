...
export default function TestSuiteList({
  ...
  function confirmDelete(suite: TestSuite) {
    ...
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
        ...
        <div className="flex items-center gap-3">
          {hiddenCount > 0 && (
            <button
              ...
              className="flex items-center gap-2 px-3 py-2 text-sm text-zinc-400 hover:text-zinc-200 bg-zinc-800 hover:bg-zinc-700 rounded-lg transition-colors"
            >
      ...
      {displayed.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="text-sm text-zinc-600 mt-1">Create your first suite to start organizing test cases</p>
          <button
            onClick={openCreate}
              ...
              <tbody>
                {displayed.map(suite => {
                  ...
                  return (
                    <tr
                      key={suite.id}
                      className={`border-b border-zinc-800/60 hover:bg-zinc-800/30 transition-colors group ${suite.isHidden ? 'opacity-50' : ''}`}
                    >
                      ...
                      <td className="px-4 py-3 text-zinc-400 text-xs">{owner?.name ?? '—'}</td>
                      <td className="px-4 py-3">
                        {suite.jiraNumber ? (
                          <span className="text-xs font-mono text-indigo-400">{suite.jiraNumber}</span>
                        ) : (
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button
                            onClick={() => onToggleHidden(suite.id)}
                            title={suite.isHidden ? 'Show suite' : 'Hide suite'}
