...
export default function UserManagement({ users, onSave, onDelete }: UserManagementProps) {
  ...
  function validate() {
    ...
    if (emailConflict) e.email = 'Email already in use'
    setErrors(e)
    ...
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      ...
      {users.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="text-sm text-zinc-600 mt-1">Add members to assign ownership and track test runs</p>
          <button
            onClick={openCreate}
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {users.map(user => (
                  ...
                  >
                    <td className="px-5 py-3 font-medium text-zinc-200">{user.name}</td>
                    <td className="px-4 py-3 text-zinc-400 text-xs">{user.email}</td>
                    <td className="px-4 py-3">
                      <div className="flex flex-wrap gap-1">
                        {user.roles.length > 0 ? (
                          user.roles.map(role => (
                            <Badge key={role} variant="role" value={role}>{role}</Badge>
                          ))
                        ) : (
      ...
      <Modal
        ...
        title={editingUser ? 'Edit Member' : 'Add Team Member'}
        size="sm"
        footer={
          <>
            <button
      ...
      >
        <div className="space-y-5">
          ...
          <div>
            ...
            <p className="text-xs text-zinc-600 mt-2">Select all that apply</p>
          </div>
        </div>
      </Modal>
