...
import type {
  User, TestSuite, TestCase, TestRun, RunAttachment,
  Defect, DefectSeverity, DefectStatus, Comment, ActivityEvent, ActivityAction,
  UserRole, TestStatus, TesterRole, InheritanceConfig,
} from '../types'
export { toTestSuite, toTestCase, fromTestCase, buildCopyTitle, generateTestCaseId } from './helpers'
import { toTestSuite, toTestCase, fromTestCase, buildCopyTitle, generateTestCaseId } from './helpers'
import type { DbTestSuite, DbTestCase, DbInheritance } from './helpers'
...
export function useTestStore() {
  ...
  const copyTestCase = useCallback(async (sourceId: string, targetSuiteId?: string) => {
    ...
    const copyTitle = buildCopyTitle(source.title, existingTitles)
